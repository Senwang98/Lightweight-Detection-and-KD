...
from ..builder import DISTILLER, build_distill_loss, build_distill_neck
from collections import OrderedDict


@DISTILLER.register_module()
class DetectionDistiller_Adap(BaseDetector):
    ...
    def __init__(self,
                 teacher_cfg,
                 student_cfg,
                 stu_feature_adap=None,
                 distill_cfg=None,
                 teacher_pretrained=None,
                 init_student=False):

        super(DetectionDistiller_Adap, self).__init__()
        ...
        self.adap_layers = build_distill_neck(stu_feature_adap)
    ...
    def forward_train(self, img, img_metas, **kwargs):
        ...
        student_feat_adap = self.adap_layers(student_feat)

        for i in range(len(student_feat_adap)):
            loss_name = 'loss_fgd_fpn_'+str(i)
            student_loss[loss_name] = self.distill_losses[loss_name](
                student_feat_adap[i], fea_t[i].detach(), kwargs['gt_bboxes'], img_metas)
